...
import { parseOnDataPlayoffSnapshotPayload } from '@/lib/ondata-playoff-contract'
import {
  hashOnDataPlayoffSnapshotPayload,
  persistOnDataPlayoffSnapshot,
} from '@/lib/ondata-playoff-server'
...
export async function POST(
  ...
  let payload
  try {
    payload = parseOnDataPlayoffSnapshotPayload(body)
  } catch (error) {
  ...
  try {
    const result = await persistOnDataPlayoffSnapshot(
      supabase,
      competition.id,
      payload,
      hashOnDataPlayoffSnapshotPayload(payload),
    )
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Kunde inte spara playoff-snapshot.' },
      { status: 500 },
    )
  }
}