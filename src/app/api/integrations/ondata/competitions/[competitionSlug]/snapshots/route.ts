...
import { parseOnDataSnapshotPayload } from '@/lib/ondata-integration-contract'
import { hashOnDataSnapshotPayload, persistOnDataSnapshot } from '@/lib/ondata-integration-server'
...
export async function POST(
  ...
  let payload
  try {
    payload = parseOnDataSnapshotPayload(body)
  } catch (error) {
  ...
  try {
    const result = await persistOnDataSnapshot(
      supabase,
      competition.id,
      payload,
      hashOnDataSnapshotPayload(payload),
    )
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Kunde inte spara snapshot.' },
      { status: 500 },
    )
  }
}
