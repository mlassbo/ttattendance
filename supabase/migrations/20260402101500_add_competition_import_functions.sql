create or replace function competition_registration_counts(
  p_competition_ids uuid[]
)
returns table(competition_id uuid, imported_registration_count bigint)
language sql stable
as $$
  select
    c.id as competition_id,
    count(r.id)::bigint as imported_registration_count
  from competitions c
  left join sessions s on s.competition_id = c.id
  left join classes cl on cl.session_id = s.id
  left join registrations r on r.class_id = cl.id
  where p_competition_ids is null or c.id = any(p_competition_ids)
  group by c.id;
$$;
