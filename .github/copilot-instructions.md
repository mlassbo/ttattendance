# TTAttendance — Copilot Instructions

This file instructs the AI agent on how to help developers set up and work with this project.
...
Secrets used for operational tasks (deployment checks, external API calls) live in `.env.agent.local`. This file is gitignored (matches `.env*.local`) and is separate from `.env.local`, which holds Next.js app variables.
...
Then fill in the values. The file is never loaded by Next.js — the agent reads it directly when needed.
